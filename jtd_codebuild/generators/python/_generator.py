...
class PythonJTDCodeGenerator(JTDCodeGenerator):
    ...
    def generate(self, target: PythonTarget) -> None:  # noqa: C901
        ...
        should_remove_json_codec_helpers = target.typingBackend in [
            "pydantic",
            "pydantic-dataclass",
            "typed-dictionary",
        ]
        should_add_json_codec_methods = target.typingBackend == "dataclass"

        # Remove root schema from the file if the option is set
        if target.removeRootSchema:
            classes = pipe(
                classes,
                filter(lambda node: node.name not in ROOT_SCHEMA_NAMES),
                list,
            )
        ...
        if should_remove_json_codec_helpers:
        ...
        for node in classes:
            ...
            node = remove_json_codec_methods(node)
...
def create_json_codec_methods(class_: ClassDef) -> ClassDef:
    ...
    methods = "\n".join(
        [
            ...
            "@classmethod",
            f"def from_json_data(cls, data: Any) -> '{class_.name}':",
            space(
                4,
                "return cls(",
            ),
...
def remove_json_codec_helpers(functions: list[FunctionDef]):
    return pipe(
        functions,
        filter(
...
def remove_json_codec_methods(class_: ClassDef):
    class_.body = pipe(
        class_.body,
